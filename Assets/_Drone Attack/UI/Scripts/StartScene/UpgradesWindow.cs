...
namespace _Drone_Attack.UI.Scripts.StartScene
{
    public class UpgradesWindow : WindowView
    {
        ...
        private void UpdateUpgradesText()
        {
            ...
            _damageProcessText.text = _playerProgressService.IsDamageLevelMax ? "MAX" : $"{Math.Round((_playerProgressService.DamageUpgrade - 1) * 100)}%";
            _recoilProcessText.text = _playerProgressService.IsRecoilLevelMax ? "MAX" : $"{Math.Round((_playerProgressService.RecoilUpgrade - 1) * 100)}%";
            _healthProcessText.text = _playerProgressService.IsHealthLevelMax ? "MAX" : $"{Math.Round((_playerProgressService.HealthUpgrade - 1) * 100)}%";
        }
